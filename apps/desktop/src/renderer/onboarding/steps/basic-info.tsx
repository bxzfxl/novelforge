...
export function BasicInfoStep({ data, onChange, isAdvanced }: BasicInfoStepProps) {
  return (
    <div className="space-y-5">
      <h2 className="text-xl font-semibold text-zinc-200">基本信息</h2>

      <div>
        <label className="text-xs text-zinc-400 mb-1 block">书名 *</label>
        <Input value={data.title || ''} onChange={e => onChange('title', e.target.value)}
          placeholder="你的小说书名" className="bg-zinc-900 border-zinc-800 text-zinc-200" />
      </div>

      <div>
        <label className="text-xs text-zinc-400 mb-1 block">笔名</label>
        <Input value={data.author || ''} onChange={e => onChange('author', e.target.value)}
          placeholder="你的笔名" className="bg-zinc-900 border-zinc-800 text-zinc-200" />
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="text-xs text-zinc-400 mb-1 block">目标字数</label>
      ...
      {isAdvanced && (
        <>
          <div>
            <label className="text-xs text-zinc-400 mb-1 block">子类型</label>
            <Input value={data.subGenre || ''} onChange={e => onChange('subGenre', e.target.value)}
              placeholder="如：废柴逆袭、凡人修仙" className="bg-zinc-900 border-zinc-800 text-zinc-200" />
          </div>
          <div>
            <label className="text-xs text-zinc-400 mb-1 block">简介</label>
            <textarea
      ...
      {!isAdvanced && (
        <div>
          <label className="text-xs text-zinc-400 mb-1 block">简介（可选）</label>
          <textarea
