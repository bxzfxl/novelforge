        ...
        <div className="max-w-3xl mx-auto flex items-center gap-1 overflow-x-auto">
          {CATEGORIES.map((label, i) => (
            <div key={i} className="flex items-center gap-1 flex-shrink-0">
              ...
              {i < CATEGORIES.length - 1 && <div className="w-4 h-px bg-zinc-800" />}
            </div>
          ))}
        </div>
      </div>
...
function SupportingCharsStep({ data, onChange }: { data: Record<string, any>; onChange: (k: string, v: any) => void }) {
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-semibold text-zinc-200">配角设定</h2>
      <p className="text-sm text-zinc-500">暂设主要配角，后续可在角色管理中详细编辑。</p>
      <textarea
        className="w-full h-32 bg-zinc-900 border border-zinc-800 rounded-lg p-4 text-sm text-zinc-300 resize-none outline-none focus:border-zinc-700"
